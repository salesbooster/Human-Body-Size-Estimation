...
#    vertex = obj2npy(gender)[0]
    ...
    for i in range(0, utils.M_NUM):
      print("This we gottt!!  "+"%s: %f" % (utils.M_STR[i], measure[i]))
    
    chest = "{:.2f}".format(measure[3][0])
    ...
    # arr = []
    # for i in range(0, utils.M_NUM):
    #   if(utils.M_STR[i] == "waist" or utils.M_STR[i] == "shoulder width" or utils.M_STR[i] == "chest" or utils.M_STR[i] == "arm length"):
    #     arr.append(measure[i])
    # print(arr)
